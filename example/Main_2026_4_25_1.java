...
简明思路：
    离散化公差
    所有可能的公差 ≤ 80×79/2 种，用 Map 映射成 0..idx-1。
    DP 状态
    dp[i][j][p]：以第 i 个元素结尾、长度 j、公差索引 p 的等差子序列数。
    初始化
    长度 1：每个元素单独成序列，dp[i][1][p] = 1（占位公差任意），答案 ans[1] = n。
    转移
    dp[i][j][p] += dp[k][j-1][p]，其中 k < i，公差 d = A[i]-A[k] 映射到 p。
    汇总
    对每个 j ≥ 2，ans[j] = Σ dp[i][j][p]（i 从 1..n，p 从 0..idx-1）。
    复杂度
    O(N³ × 不同公差数) ≈ 80³ × 3160，约 1.6e9 理论上偏大，但通过剪枝（j ≤ i 且只枚举有效 k）和常数优化可过。实际可用二维滚动或优化循环顺序。
*/
